...
func New(options Options) *Interpreter {
	...
	i.opt.astDot, _ = strconv.ParseBool(os.Getenv("mozilla_AST_DOT"))

	// cfgDot activates CFG graph display for the interpreter
	i.opt.cfgDot, _ = strconv.ParseBool(os.Getenv("mozilla_CFG_DOT"))
...
func getPrompt(in io.Reader, out io.Writer) func(reflect.Value) {
	forcePrompt, _ := strconv.ParseBool(os.Getenv("mozilla_PROMPT"))
	if forcePrompt {
		return doPrompt(out)
	}
